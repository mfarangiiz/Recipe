            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('recipes.update',$recipe->id) }}" enctype="multipart/form-data">
                        ...
                        <x-input-label for="" :value="__('isim familiya')" />
                        <x-text-input id="" name="user" type="text" value="{{$recipe->user}}" class="mt-1 block w-full" autocomplete="" />
                        <x-input-error :messages="$errors->get('user')" class="mt-2" />

                        <x-input-label for="" :value="__('NOMI')" />
                        <x-text-input id="" name="name" type="text" value="{{$recipe->name}}" class="mt-1 block w-full" autocomplete="" />
                        ...
                        <x-text-input id="" name="instructions" value="{{$recipe->instructions}}" type="text" class="mt-1 block w-full" autocomplete="" />
                        <x-input-error :messages="$errors->get('instructions')" class="mt-2" />

                        <x-input-label for="" :value="__('Rasim')" />
